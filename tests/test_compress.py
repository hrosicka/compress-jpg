...
def test_successful_resize(mock_image, capsys):
    ...
    assert (
        f"The image was resized by {percentage}% and saved to: {output_path}"
        in captured.out
    )


@pytest.mark.parametrize(
    "percentage, expected_dims",
    [
...
def test_no_resize_for_large_percentage(mock_image, capsys, percentage):
    ...
    captured = capsys.readouterr()
    assert (
        "The entered percentage is greater than or equal to 100%. No resizing will be performed."
        in captured.out
    )
...
def test_generic_processing_error(mock_image, mock_image_open, capsys):
    ...
    captured = capsys.readouterr()
    assert (
        "An error occurred during image processing: Simulated processing error"
        in captured.out
    )
...
def test_main_script_input_validation(mock_input, mock_resize_func, capsys):
    ...
    assert (
        captured.out.count(
            "Invalid input. Please enter a numerical value for the percentage."
        )
        == 1
    )
    assert (
        captured.out.count(
            "The resizing percentage must be greater than 0 and less than 100 for proper reduction."
        )
        == 2
    )
